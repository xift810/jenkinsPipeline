pipeline {
    ...
    environment {
        CI_PROJECT_DIR = 'E:\\k12jenkins\\playgroundPython'
        ...
        stage('UploadPlugin'){
            steps {
                powershell """
                    ...
                    python $Scripts_Folder\\uploadfile.py --uploadPrefix=PlaygroundLib/Launcher/$CI_COMMIT_TAG --filePath=$CI_PROJECT_DIR/Unity.InternalAPIEditorBridge.001.dll --isPublic=True
                    python $Scripts_Folder\\downloadfile.py --fileName=PlaygroundPlugin/Temp/UnityK12Playground_windows.unitypackage --dest=$CI_PROJECT_DIR/UnityK12Playground_windows.unitypackage
                    python $Scripts_Folder\\uploadfile.py --uploadPrefix=PlaygroundPlugin/$CI_COMMIT_TAG --filePath=$CI_PROJECT_DIR/UnityK12Playground_windows.unitypackage --isPublic=True
